...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    return () => ctx.revert();
  }, []); // GSAP dependencies updated for CSS smooth transition
        ...
        <div className="search-top-block w-full flex flex-col items-center mb-10">
          ...
          <div className="flex flex-col md:flex-row items-center w-full max-w-[900px] border border-gray-200 rounded-2xl md:rounded-full bg-white shadow-[0_4px_20px_rgb(0,0,0,0.04)] focus-within:ring-2 focus-within:ring-brand-orange/40 transition-all">
            ...
            <div className="flex-1 flex items-center px-6 h-[64px] w-full relative group">
              ...
              <button className="w-9 h-9 rounded-full bg-slate-50 hover:bg-slate-100 flex items-center justify-center shrink-0 transition-colors">
                <Crosshair className="w-[18px] h-[18px] text-brand-navy opacity-60 hover:opacity-100" />
              </button>
            </div>
          ...
          {/* Filters Toggle Button (Border Removed & Shadow Added) */}
          ...
          {/* EXPANDED FILTERS PANEL (Smooth CSS Grid Animation) */}
          <div 
